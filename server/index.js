...
app.get('/api/download', async (req, res) => {
  ...
  try {
    const rawTitle = (title || 'Track').trim();
    const rawArtist = (artist || 'Suno Music').trim();
    const cleanFilename = `${rawTitle} - ${rawArtist}`
      .replace(/[\/\\?%*:|"<>]/g, '_')
      .trim() + '.mp3';
    ...
    if (!res.headersSent) {
      res.status(500).json({ error: 'Failed to process download', message: err.message });
    }
  }
});
...
app.post('/api/radio/similar-queue', async (req, res) => {
  const { seedSong, candidateTracks = [], recentSongs = [], playedTitles = [] } = req.body;
  try {
    const result = await generateSimilarMoodQueue(seedSong, candidateTracks, recentSongs, playedTitles);
    res.json(result);
  } catch (err) {
